...
void correct_step(real pos[][NDIM], real vel[][NDIM], const real acc[][NDIM],
                  const real jerk[][NDIM], const real old_pos[][NDIM],
                  const real old_vel[][NDIM], const real old_acc[][NDIM],
                  const real old_jerk[][NDIM], int n, real dt);
void evolve(const real mass[], real pos[][NDIM], real vel[][NDIM], int n,
            real &t, real dt_param, real dt_dia, real dt_out, real dt_tot,
            bool init_out, bool x_flag);
void evolve_step(const real mass[], real pos[][NDIM], real vel[][NDIM],
                 real acc[][NDIM], real jerk[][NDIM], int n, real &t, real dt,
...
void predict_step(real pos[][NDIM], real vel[][NDIM], const real acc[][NDIM],
                  const real jerk[][NDIM], int n, real dt);
...
void write_diagnostics(const real mass[], const real pos[][NDIM],
                       ...
                       int nsteps, real &einit, bool init_flag, bool x_flag);
...
void write_diagnostics(const real mass[], const real pos[][NDIM],
    ...
    cerr << "at time t = " << t << " , after " << nsteps
         ...
         << " , E_tot = " << etot << endl;
    cerr << " "
         << "absolute energy error: E_tot - E_init = " << etot - einit << endl;
    cerr << " "
...
void evolve_step(const real mass[], real pos[][NDIM], real vel[][NDIM],
    ...
    get_acc_jerk_pot_coll(mass, pos, vel, acc, jerk, n, epot, coll_time);
    correct_step(pos, vel, acc, jerk, old_pos, old_vel, old_acc, old_jerk, n,
                 dt);
    t += dt;
    delete[] old_pos;
    delete[] old_vel;
...
void get_acc_jerk_pot_coll(const real mass[], const real pos[][NDIM],
    ...
    for (int i = 0; i < n; i++) {
        for (int j = i + 1; j < n; j++) { // rji[] is the vector from
            ...
            // first collision time estimate, based on unaccelerated linear
            // motion:
